...
public partial class App
{
	...
	protected override void OnStartup(StartupEventArgs e)
	{
		...
		ServiceLocator.GetService<SkinManagerService>().LoadSkinConfig(SettingService.GetSettingValue("Theme")
			                                                               .ToString()!);
	}
	...
	private static void RegisterServices(UnityContainer container)
	{
		...
		container.RegisterType<INavigationService, FrameNavigationService> // 数据管理页面导航服务
			("DataContentFrameNavigationService",
			 new ContainerControlledLifetimeManager());
		container.RegisterType<ConfigurationService>("PagesConfigService", // 页面配置服务
		                                             new ContainerControlledLifetimeManager(),
